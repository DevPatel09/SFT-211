...
struct StringIndex indexString(const char* str)
{
	...
	for (i = 0; str[i] != '\0'; i++)
	{
		while (str[i] != '\0' && isspace(str[i]))
		{
			if (str[i] == '\n')
			{
				result.lineStarts[result.numLines++] = i + 1;   // CORRECTED CODE
				//result.lineStarts[result.numLines] = i + 1;   // BUG CODE
			}
			i++;
		}
		sp = nextWhite(str + i);
...
void getWord(char word[], const struct StringIndex* idx, int wordNum)
{
	...
	if (wordNum < idx->numWords && wordNum >= 0)
	{
		...
		for (i = 0; i < sp - start; i++)  // CORRECTED CODE
			//for (i = 0; i < sp; i++)      // BUG CODE
		{
			word[i] = idx->str[start + 1];
				//word[i] = idx->str[i];    // BUG CODE
		}
	...
	numberNum)
{
	...
	if (numberNum < idx->numNumbers && numberNum >= 0)
	{
		...
		sp = nextWhite(idx->str + start);
		for (i = 0; i < sp - start; i++) // CORRECTED CODE
		//for (i = 0; i < sp; i++)       // CAUSING A BUG
		{
			word[i] = idx->str[start + i];
		}
		word[i] = '\0';        // CORRECTED CODE
		//word[sp] = '\0';     // CASUING A BUG
	}
}
